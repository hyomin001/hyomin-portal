...
def render(market, nw):
    ...
    with tab_market:
        ...
        for s in stock_config:
            ...
            rows += f"<tr><td>{s['icon']} {d['name']}</td><td style='text-align:right;font-weight:900;color:#E2E8F0;'>₩{d['price']:,}</td><td class='{cls}' style='text-align:right;'>{arr} {abs(pct):.2f}%</td><td style='text-align:right;color:#B0BAC8;'>₩{d['history'][-2]:,}</td></tr>"
    ...
    with tab_trade:
        ...
        if my_qty > 0:
            ...
            st.markdown("<div style='background:rgba(255,255,255,0.03);border:1px dashed rgba(255,255,255,0.1);padding:12px;border-radius:10px;margin-bottom:18px;color:#B0BAC8;text-align:center;'>현재 보유 중인 주식이 없습니다.</div>", unsafe_allow_html=True)
        ...
        st.markdown(f"<div style='color:#B0BAC8;font-size:0.78rem;margin-bottom:8px;'>풀매수/풀매도 오늘 남은 횟수: <b style='color:#FFD600;'>{bulk_left}회</b></div>", unsafe_allow_html=True)

        # ─────────────────────────────────────────────────────────────────
        # FIX: _safe_buy — 매수 시 DB 포트폴리오 재검증 추가
        #   기존: 현금만 DB 재확인, 포트폴리오 수량은 세션만 참조
        #   변경: DB에서 현재 포트폴리오 수량도 읽어 세션과 비교 후
        #         더 작은 값을 신뢰 (다중 탭 동시 매수 방지)
        # ─────────────────────────────────────────────────────────────────
        def _safe_buy(qty, price, sid_):
            ...
            u_me = u_db.get(st.session_state.logged_in_user, {})
            db_cash = u_me.get('cash', 0)

            if db_cash < total:
                st.error("잔액 부족! (DB 검증 실패)"); return False

            # 매수는 포트폴리오 수량 제한이 없으므로 현금만 DB 기준으로 차감
