...
if (IS_EGG) {{
  // ── EGG stage
  const eggAnim = EXP_PCT >= 80 ? 'eggShake 0.18s ease-in-out infinite'
                                 : 'eggWobble 2.5s ease-in-out infinite';
  const innerOp = EXP_PCT>=80 ? '0.75' : EXP_PCT>=60 ? '0.45' : EXP_PCT>=40 ? '0.22' : '0';
  const crackLightOp = CRACK4 ? '1' : '0';

  center.innerHTML = `
    <div id="egg-wrap" onclick="eggClick(event)">
      <svg id="egg-svg" viewBox="0 0 120 148" width="165" height="202"
           xmlns="http://www.w3.org/2000/svg"
           style="animation:${{eggAnim}};filter:drop-shadow(0 0 24px ${{RAR_COLOR}})">
        <defs>
          ...
          <radialGradient id="ig" cx="50%" cy="50%" r="50%">
            <stop offset="0%" stop-color="${{RAR_COLOR}}" stop-opacity="0.7"/>
            <stop offset="100%" stop-color="${{RAR_COLOR}}" stop-opacity="0"/>
          </radialGradient>
          <filter id="ds"><feDropShadow dx="0" dy="6" stdDeviation="10"
            flood-color="${{RAR_COLOR}}" flood-opacity="0.6"/></filter>
          <filter id="cshadow">
            <feDropShadow dx="0" dy="0" stdDeviation="3"
              flood-color="${{RAR_COLOR}}" flood-opacity="0.9"/>
          </filter>
        </defs>
        <!-- Egg body -->
        <ellipse cx="60" cy="83" rx="50" ry="70" fill="url(#eg)" filter="url(#ds)"/>
        <!-- Inner glow -->
        <ellipse cx="60" cy="83" rx="45" ry="64" fill="url(#ig)"
          opacity="${{innerOp}}" style="animation:innerGlowPulse 0.9s ease-in-out infinite"/>
        <!-- Highlights -->
        <ellipse cx="40" cy="48" rx="14" ry="20" fill="white" opacity="0.38"
          transform="rotate(-22,40,48)"/>
        <ellipse cx="72" cy="37" rx="5" ry="7" fill="white" opacity="0.22"/>
        <!-- Crack 1: top center -->
        ${{CRACK1 ? `<polyline points="65,18 70,33 62,46 68,57"
          stroke="#7a3a05" stroke-width="2.5" fill="none"
          stroke-linecap="round" stroke-linejoin="round"
          style="animation:crackGlow 1s ease-in-out infinite"/>` : ''}}
        <!-- Crack 2: right side -->
        ${{CRACK2 ? `<polyline points="96,72 88,85 93,98 86,112"
          stroke="#7a3a05" stroke-width="2" fill="none"
          stroke-linecap="round" stroke-linejoin="round"/>` : ''}}
        <!-- Crack 3: left side -->
        ${{CRACK3 ? `<polyline points="24,68 32,81 26,95 33,108"
          stroke="#7a3a05" stroke-width="2" fill="none"
          stroke-linecap="round" stroke-linejoin="round"/>` : ''}}
        <!-- Crack 4: major crack with glow -->
        ${{CRACK4 ? `
          <polyline points="52,12 57,28 50,46 58,62 52,78 58,94 52,110 57,126"
            stroke="#4a1a00" stroke-width="4" fill="none"
            stroke-linecap="round" stroke-linejoin="round"/>
          <polyline points="52,12 57,28 50,46 58,62 52,78 58,94 52,110 57,126"
            stroke="${{RAR_COLOR}}" stroke-width="1.5" fill="none"
            stroke-linecap="round" stroke-linejoin="round" opacity="0.95"
            filter="url(#cshadow)"
            style="animation:crackGlow 0.5s ease-in-out infinite"/>
          <line x1="58" y1="62" x2="78" y2="55" stroke="#4a1a00" stroke-width="3"
            stroke-linecap="round"/>
          <line x1="58" y1="62" x2="78" y2="55" stroke="${{RAR_COLOR}}" stroke-width="1.2"
            stroke-linecap="round" opacity="0.9"/>
          <line x1="52" y1="78" x2="32" y2="72" stroke="#4a1a00" stroke-width="2.5"
            stroke-linecap="round"/>
        ` : ''}}
        <!-- Border -->
        <ellipse cx="60" cy="83" rx="50" ry="70" fill="none"
          stroke="${{RAR_COLOR}}" stroke-width="1.5" opacity="0.5"/>
      </svg>
      <div id="hatch-info">
        <div id="hatch-lbl">${{EXP_PCT>=80 ? '💥 부화 임박!!!' : '🥚 부화까지 ' + REMAINING_PCT + '% 남음'}}</div>
        <div id="hatch-bar-bg">
          <div id="hatch-fill" style="width:${{EXP_PCT}}%;${{EXP_PCT>=80?'animation:innerGlowPulse 0.4s infinite;':''}}"></div>
        </div>
      </div>
    </div>`;
  ...
  window.eggClick = function(e) {{
    ...
    spawnSparks(e.clientX-rect.left, e.clientY-rect.top, 6);
    for(let i=0;i<5;i++)
      setTimeout(()=>spawnHeart(
        e.clientX-rect.left+(Math.random()-0.5)*60,
        e.clientY-rect.top+(Math.random()-0.5)*30
      ), i*90);
  }};

}} else {{
  // ── PET stage
  const moodCls = HAPPY>=80&&HUNGER>=70 ? 'mood-happy'
                : IS_LEGEND ? 'mood-legend'
                : HP_VAL<30||HUNGER<20 ? 'mood-sleep'
                : HAPPY<40 ? 'mood-sad' : 'mood-default';

  const wings = HAS_WINGS ? `
    <span class="wing wing-l">${{WING_EMOJI}}</span>
    <span class="wing wing-r">${{WING_EMOJI}}</span>` : '';

  const rings = IS_LEGEND ? `
    <div id="ring1"></div><div id="ring2"></div><div id="ring3"></div>` : '';

  center.innerHTML = `
    ${{rings}}
    <div id="pet-area" class="${{moodCls}}">
      ${{wings}}
  ...
  petArea.addEventListener('click', e => {{
    const r=APP.getBoundingClientRect();
    spawnSparks(e.clientX-r.left, e.clientY-r.top, 8);
    for(let i=0;i<10;i++)
      setTimeout(()=>spawnHeart(
        e.clientX-r.left+(Math.random()-0.5)*80,
        e.clientY-r.top+(Math.random()-0.5)*45
      ), i*70);
    // Dragon fire breath
    if(PET_SPECIES==='dragon') spawnFireBreath(e.clientX-r.left, e.clientY-r.top);
    // Phoenix embers
