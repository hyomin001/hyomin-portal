...
def render():
    ...
    # ── 게임 결과 처리 ──
    _qp = st.query_params
    if _qp.get('fighter_score') and not st.session_state.get('_fighter_saved'):
        st.session_state['_fighter_saved'] = True
        try:
            _uid       = _qp.get('_gr_uid', _cur_uid) or _cur_uid
            _f_score   = int(_qp.get('fighter_score', 0))
            _f_perfects = int(_qp.get('fighter_perfects', 0))
            if _f_score > 0 and _uid:
                _col  = _get_col(USERS_FILE)
                _doc  = _col.find_one({"_id": "main"}, {_uid: 1})
                if _doc and _uid in _doc:
                    _udata = _doc[_uid]
                    _cur_best = _udata.get('game_records', {}).get('fighter', {}).get('score', 0)
                    if _f_score > _cur_best:
                        _col.update_one(
                            {"_id": "main"},
                            {"$set": {
                                f"{_uid}.game_records.fighter.score":    _f_score,
                                f"{_uid}.game_records.fighter.perfects": _f_perfects,
                            }}
                        )
                        update_leaderboard('fighter', _udata.get('nickname', _uid), _f_score)
        ...
        st.rerun()
    elif not _qp.get('fighter_score'):
