...
<div id="root">
  ...
  <div class="overlay" id="halftimeOverlay" style="display:none;">
    <div class="ov-box">
      <div class="ov-title" style="font-size:24px;">⏱️ HALF TIME</div>
      <div id="ht-score" style="font-family:'Black Han Sans',sans-serif;font-size:36px;margin:6px 0;">0 : 0</div>
      <div id="htBody"></div>
    </div>
  </div>
</div>
...
const W=1040,H=660, H0=40,H1=1340, V0=20,V1=1020;
let camX=0, camInit=false;
const GY0=401, GY1=639; // goal mouth
...
function inOwnBox(p,team){
  if(team==='B') return p.x<H0+240 && p.y>GY0-123 && p.y<GY1+123;
  return p.x>H1-240 && p.y>GY0-123 && p.y<GY1+123;
}
...
function doLongOrCross(passer){
  const nearByline = passer.team==='B' ? passer.x>H1-352 : passer.x<H0+352;
...
function doShoot(passer,power){
  ...
  const aimY = clamp(CY+off, GY0+20, GY1-20);
...
function isInjured(p){
  return p && p.injuredUntilMatchday && careerState && p.injuredUntilMatchday > careerState.league.matchday;
}
...
function pickSquadForFormation(squad, formationName){
  ...
  squad.forEach(pl=>{ if(byRole[pl.role] && !isInjured(pl)) byRole[pl.role].push(pl); });
  ...
  for(const role of ['GK','DF','MF','FW']){
    ...
    while(picks.length<need[role]) picks.push(genSquadPlayer(role,58)); // 부상자 대체용 비상 유스 선수
    chosen.push(...picks);
  }
  return chosen;
}
function getLineupForFormation(formationName){
  ...
  // 저장된 라인업 검증: id 존재 + 역할 일치 + 중복 없어야 유효 + 부상 아니어야 유효
  if(saved && saved.length===11){
    const used=new Set(); let valid=true;
    for(let i=0;i<11;i++){
      const pid=saved[i];
      const pl=squadById[pid];
      if(!pid || !pl || pl.role!==bySlotRole[i] || used.has(pid) || isInjured(pl)){ valid=false; break; }
      used.add(pid);
    }
    if(valid) return saved.slice();
  }
  // 유효하지 않으면 자동 베스트11(부상자 제외)로 채우고 저장
...
let halftimeDone=false, subsUsedThisMatch=0, benchUsedIds=new Set();
const SUBS_MAX=3;
...
function careerCreate(name, crest){
  ...
  careerState = {
    ...
    trophies: [],
    continentalEligible: false,
    continentalDone: false,
    continental: null,
...
function careerOnMatchEnd(b,r,win){
  ...
  if(typeof blue!=='undefined' && blue){
    for(const p of blue){
      ...
      const sq=careerState.squad.find(s=>s.id===p.squadId);
      if(sq){
        sq.seasonGoals=(sq.seasonGoals||0)+(p.matchGoals||0);
        sq.seasonAssists=(sq.seasonAssists||0)+(p.matchAssists||0);
        sq.seasonTackles=(sq.seasonTackles||0)+(p.matchTackles||0);
      }
    }
  }

  const stadiumLv = careerState.facilities? careerState.facilities.stadium : 1;
  const gateIncome = 800000 * stadiumLv * (win?1.4:1);
  careerState.budget += Math.round(gateIncome);

  // 부상 판정 (8% 확률로 출전 선수 중 1명)
  if(typeof blue!=='undefined' && blue && Math.random()<0.08){
    const withSquad = blue.filter(p=>p.squadId);
    if(withSquad.length){
      const victim = withSquad[Math.floor(Math.random()*withSquad.length)];
      const squadP = careerState.squad.find(s=>s.id===victim.squadId);
      if(squadP){
        const duration = 1+Math.floor(Math.random()*3);
        squadP.injuredUntilMatchday = careerState.league.matchday + duration;
        commentate(`🚑 ${squadP.name} 부상! ${duration}경기 결장 예상`);
      }
    }
  }
...
function careerSeasonEnd(){
  ...
  // ── 시즌 어워드 계산 (성장/은퇴 처리 전에 이번 시즌 기록 기준으로) ──
  careerState.trophies = careerState.trophies || [];
  let topScorerInfo=null, bestYoungInfo=null, playerOfSeasonName=null;
  if(careerState.squad.length){
    const topScorer = [...careerState.squad].sort((a,b)=>(b.seasonGoals||0)-(a.seasonGoals||0))[0];
    if(topScorer) topScorerInfo = {name:topScorer.name, goals:topScorer.seasonGoals||0};
    const youngsters = careerState.squad.filter(p=>p.age<=21);
    if(youngsters.length){
      const bestYoung = [...youngsters].sort((a,b)=>ovrOf(b)-ovrOf(a))[0];
      bestYoungInfo = {name:bestYoung.name, age:bestYoung.age, ovr:ovrOf(bestYoung)};
    }
    const playerOfSeason = [...careerState.squad].sort((a,b)=>{
      const sa=(a.seasonGoals||0)*2+(a.seasonAssists||0)+(a.seasonTackles||0)*0.3;
      const sb=(b.seasonGoals||0)*2+(b.seasonAssists||0)+(b.seasonTackles||0)*0.3;
      return sb-sa;
    })[0];
    if(playerOfSeason) playerOfSeasonName = playerOfSeason.name;
  }
  if(myPos===1){
    careerState.trophies.push(`${TIER_NAMES[careerState.tier - tierChange]} 우승 (시즌 ${careerState.season})`);
  }
  careerState.continentalEligible = (myPos<=4);
  careerState.continentalDone = false;
  careerState.continental = null;

  const retired=[], departed=[];
  careerState.squad.forEach(p=>{
    p.age++;
    p.injuredUntilMatchday=0; // 오프시즌 휴식으로 전원 완치
    p.seasonGoals=0; p.seasonAssists=0; p.seasonTackles=0;
  ...
  const scoutLv = careerState.facilities? careerState.facilities.scouting : 1;
  const [omin,omax]=tierOvrRange(careerState.tier);
  const marketSize = 10 + (scoutLv-1)*3;
  const qualityBoost = (scoutLv-1)*6;
  careerState.market = genMarketPool(marketSize, omin, omax+qualityBoost);
  let academyProspect=null;
  const academyChance = 0.45 + (scoutLv-1)*0.15;
  if(Math.random()<academyChance){
  ...
  return {myPos, prize, retired, departed, tierChange, academyProspect,
    ...
    bestYoung: bestYoungInfo,
    playerOfSeason: playerOfSeasonName
  };
}
...
function careerSquadRowsHtml(){
  ...
  return rows.map(p=>{
    ...
    const remain = inj? (p.injuredUntilMatchday - careerState.league.matchday) : 0;
    return `<tr><td class="tname">${p.custom?'✨ ':''}${p.name}${inj?` <span style="color:#ff4757;font-size:9.5px;">🚑${remain}경기</span>`:''}</td><td>${p.role}</td><td>${p.age}</td><td><b>${ovrOf(p)}</b></td><td>${fmtMoney(playerValue(p))}</td><td>${p.contract||1}년</td>
    <td><div class="mini-btn" data-action="career_sell" data-id="${p.id}">방출</div></td></tr>`;
  }).join('');
}
...
function renderCareerHub(){
  ...
  if(careerState.league.done){
    setMetaBody(`
      ...
      <div class="ghost-btn" data-action="career_trophies">🏆 트로피 캐비닛 (${(careerState.trophies||[]).length})</div>
      <div class="ghost-btn" data-action="goto_hub">◀ 메인으로 (진행상황 저장됨)</div>
    `);
    return;
  }
  ...
  setMetaBody(`
    ...
    ${(careerState.continentalEligible && !careerState.continentalDone)? '<button class="continue-btn" data-action="career_continental" style="margin-top:8px;background:linear-gradient(90deg,#7b2ff7,#f107a3);">🌍 대륙컵 참가</button>' : ''}
    <div class="diff-grid" style="margin-top:8px;grid-template-columns:repeat(5,1fr);">
      <div class="opt-btn" data-action="career_lineup">🧩 라인업</div>
      <div class="opt-btn" data-action="career_training">🏋️ 성장</div>
      <div class="opt-btn" data-action="career_squad">👥 스쿼드</div>
      <div class="opt-btn" data-action="career_market">💰 이적</div>
      <div class="opt-btn" data-action="career_facilities">🏗️ 시설</div>
    </div>
    <div class="ghost-btn" data-action="career_trophies">🏆 트로피 캐비닛 (${(careerState.trophies||[]).length})</div>
    <div class="ghost-btn" data-action="goto_hub">◀ 메인으로 (진행상황 저장됨)</div>
  `);
...
function trainCost(p){
  const ovr=ovrOf(p);
  const base = 8 + Math.floor(Math.max(0,ovr-55)/4)*2;
  const trainLv = (careerState && careerState.facilities)? careerState.facilities.training : 1;
  const discount = 1 - (trainLv-1)*0.15; // Lv2: -15%, Lv3: -30%
  return Math.max(3, Math.round(base*discount));
}
...
const FACILITY_INFO={
  training:{label:'🏋️ 훈련장', desc:(lv)=> lv>=3? '훈련 비용 -30% (최대)' : `훈련 비용 -${(lv-1)*15+15}% (다음 레벨)`},
  scouting:{label:'🔭 스카우팅망', desc:(lv)=> lv>=3? '이적시장 최대 규모/품질' : '이적시장 선수 수·품질 향상 (다음 레벨)'},
  stadium:{label:'🏟️ 홈구장', desc:(lv)=> lv>=3? '입장수익 최대' : '매 경기 입장수익 증가 (다음 레벨)'}
};
function facilityCost(curLv){ return [0, 8000000, 20000000][curLv] || 99999999; }
const CONTINENTAL_CLUB_NAMES=['AC 로열스','스포르팅 노바','FC 인터내셔널','아틀란타 유나이티드FC','클럽 아즈텍','올림픽 마르세유노바','FK 다이나모','벤피카 스타즈'];
function continentalStart(){
  metaMode='continental';
  const names=shuffle(CONTINENTAL_CLUB_NAMES).slice(0,3);
  const ovrBase = 68+(careerState.tier||2)*4;
  const opponents = names.map(n=>({name:n, ovr: ovrBase+Math.round(Math.random()*14)}));
  const [ga,gb]=simScoreline(opponents[1].ovr, opponents[2].ovr);
  const otherFinalist = ga>=gb? opponents[1].name : opponents[2].name;
  careerState.continental = { opponents, stage:'SF', roundOpp:opponents[0].name, roundOpp_ovr:opponents[0].ovr, otherFinalist };
  persistCareer();
  renderCareerContinental();
}
function continentalPlayMatch(){
  const c=careerState.continental;
  blueRosterSource='career';
  aiDifficulty = c.stage==='F'? 1.18 : 1.1;
  opponentOVR = c.stage==='F'? (careerState.continental._finalOvr||78) : c.roundOpp_ovr;
  hud('r', c.roundOpp);
  startRealMatch(c.roundOpp);
}
function continentalOnMatchEnd(b,r,win){
  const c=careerState.continental;
  if(c.stage==='SF'){
    if(win){
      c.stage='F';
      c.roundOpp=c.otherFinalist;
      c.roundOpp_ovr = 68+(careerState.tier||2)*4+Math.round(Math.random()*14);
      c._finalOvr=c.roundOpp_ovr;
      persistCareer();
      return '🏁 결승 진출 확인';
    } else {
      careerState.continentalDone=true;
      persistCareer();
      return '📋 결과 확인';
    }
  } else if(c.stage==='F'){
    ...
    if(win){
      careerState.trophies = careerState.trophies||[];
      careerState.trophies.push(`대륙컵 우승 (시즌 ${careerState.season})`);
      careerState.budget += 20000000;
      c.stage='champion';
    } else {
      c.stage='runnerup';
    }
...
function renderCareerContinental(){
  metaMode='continental';
  if(!careerState.continentalEligible){
    setMetaBody(`
      <div class="meta-title">🌍 대륙컵</div>
      <div class="meta-sub">리그 4위 이내로 시즌을 마치면 다음 시즌 대륙컵 참가 자격을 얻어.</div>
      <div class="ghost-btn" data-action="career_hub">◀ 구단 사무실로</div>
    `);
    return;
  }
  if(careerState.continentalDone){
    const c=careerState.continental||{};
    const resultMsg = c.stage==='champion'? '🏆 대륙컵 우승을 차지했어!' : (c.stage==='runnerup'? '🥈 결승에서 아쉽게 패배했어.' : '😢 4강에서 탈락했어.');
    setMetaBody(`
      <div class="meta-title">🌍 대륙컵 - 시즌 종료</div>
      <div class="meta-sub">${resultMsg}</div>
      <div class="ghost-btn" data-action="career_hub">◀ 구단 사무실로</div>
    `);
    return;
  }
  if(!careerState.continental) continentalStart();
  const c=careerState.continental;
  const stageLabel = c.stage==='SF'? '4강':'결승';
  setMetaBody(`
    <div class="meta-title">🌍 대륙컵 - ${stageLabel}</div>
    <div class="meta-sub">${careerState.clubName}, 유럽 무대에 도전한다!</div>
    <div class="fixture-box">상대: <b>${c.roundOpp}</b> (OVR ${c.roundOpp_ovr})</div>
    <button class="continue-btn" data-action="continental_play">⚽ ${stageLabel} 경기 시작</button>
    <div class="ghost-btn" data-action="career_hub">◀ 구단 사무실로</div>
  `);
}

function renderCareerTrophies(){
  metaMode='career';
  const trophies = careerState.trophies||[];
  const rows = trophies.length? trophies.map(t=>`<div class="fixture-box">🏆 ${t}</div>`).join('') : `<div class="fixture-box" style="color:#8aa;text-align:center;">아직 트로피가 없어. 리그 우승을 노려보자!</div>`;
  setMetaBody(`
    <div class="meta-title">🏆 트로피 캐비닛</div>
    <div class="meta-sub">${careerState.crest} ${careerState.clubName}의 영광의 기록</div>
    ${rows}
    <div class="ghost-btn" data-action="career_hub">◀ 구단 사무실로</div>
  `);
}

function renderCareerFacilities(msg){
  metaMode='career';
  const f=careerState.facilities||{training:1,scouting:1,stadium:1};
  const rows = Object.keys(FACILITY_INFO).map(key=>{
    const lv=f[key]||1;
    const info=FACILITY_INFO[key];
    const maxed = lv>=3;
    const cost = facilityCost(lv);
    return `<tr><td class="tname">${info.label}</td><td>Lv.${lv}/3</td><td style="font-size:10px;color:#8aa;">${info.desc(lv)}</td>
      <td><div class="mini-btn ${maxed?'disabled':''}" data-action="career_facility_up" data-key="${key}">${maxed?'MAX':fmtMoney(cost)+' 투자'}</div></td></tr>`;
  }).join('');
  setMetaBody(`
    <div class="meta-title">🏗️ 시설 투자</div>
    <div class="meta-sub">보유 예산: <b style="color:var(--gold);">${fmtMoney(careerState.budget)}</b>${msg? ' · '+msg:''}</div>
    <table class="meta-table"><tr><th>시설</th><th>레벨</th><th>효과</th><th></th></tr>${rows}</table>
    <div class="ghost-btn" data-action="career_hub">◀ 구단 사무실로</div>
  `);
}
function careerFacilityUpgrade(key){
  const f=careerState.facilities=careerState.facilities||{training:1,scouting:1,stadium:1};
  const lv=f[key]||1;
  if(lv>=3) return '이미 최대 레벨이야.';
  const cost=facilityCost(lv);
  if(careerState.budget<cost) return `예산이 부족해 (필요 ${fmtMoney(cost)}).`;
  careerState.budget-=cost;
  f[key]=lv+1;
  persistCareer();
  return `✅ ${FACILITY_INFO[key].label} Lv.${lv+1} 달성!`;
}
...
function renderCareerSeasonEndResult(res){
  ...
  setMetaBody(`
    ...
    <div class="fixture-box" style="line-height:1.8;">
      <b style="color:var(--gold);">🏅 시즌 어워드</b><br>
      ⚽ 팀 득점왕: ${res.topScorer? `${res.topScorer.name} (${res.topScorer.goals}골)` : '—'}<br>
      🌟 영플레이어: ${res.bestYoung? `${res.bestYoung.name} (${res.bestYoung.age}세, OVR ${res.bestYoung.ovr})` : '—'}<br>
      👑 올해의 선수: ${res.playerOfSeason||'—'}
    </div>
...
document.getElementById('metaBox').addEventListener('click',(e)=>{
  ...
  else if(act==='career_facilities') renderCareerFacilities();
  else if(act==='career_trophies') renderCareerTrophies();
  else if(act==='career_continental') renderCareerContinental();
  else if(act==='continental_play') continentalPlayMatch();
  else if(act==='career_facility_up'){ const m=careerFacilityUpgrade(el.dataset.key); renderCareerFacilities(m); }
...
document.getElementById('halftimeOverlay').addEventListener('click',(e)=>{
  const el=e.target.closest('[data-action]');
  if(!el) return;
  const act=el.dataset.action;
  if(act==='ht_tactic'){
  ...
  else if(act==='ht_pick_out') renderHalftime(parseInt(el.dataset.slot,10));
  else if(act==='ht_back') renderHalftime();
  else if(act==='ht_sub_confirm'){
    const msg=performSub(parseInt(el.dataset.slot,10), el.dataset.id);
    renderHalftime();
    if(msg) commentate(msg);
  }
  else if(act==='ht_resume') resumeSecondHalf();
});
...
function aiCarrierDecision(p,dt){
  ...
  if(distGoal<278 && pressure>26 && Math.random()<0.022*diff){
...
function aiStep(p,dt){
  ...
  if(p.isGK){
    p.aiState='GK_HOME';
    tx = p.team==='B'? H0+34 : H1-34;
    ty = clamp(ball.y, GY0+32, GY1-32);
    spd=94;
...
function humanStep(dt){
  ...
  if(p.isGK && !p.rushBoost){
    p.x=clamp(p.x, H0-2, H0+271);
    if(p.team==='R') p.x=clamp(p.x, H1-271, H1+2);
  } else {
    clampP(p);
  }
...
function ballStep(dt){
  ...
  if(!ball.dangerChecked){
    if(ball.x<H0+176 && ball.vx<0){
      ball.dangerChecked=true;
      tryGKSave(blue.find(p=>p.isGK), true);
    } else if(ball.x>H1-176 && ball.vx>0){
      ball.dangerChecked=true;
...
const GOAL_H3D=0.9;
const CAM_HEIGHT=12, CAM_BACK=14.5;
...
function makePitchTexture(){
  ...
  tctx.beginPath(); tctx.arc(tw/2,th/2, 92*sx, 0, Math.PI*2); tctx.stroke();
  tctx.beginPath(); tctx.arc(tw/2,th/2,4,0,Math.PI*2); tctx.fillStyle='#fff'; tctx.fill();

  tctx.lineWidth=3;
  tctx.strokeRect(X(H0), Y(GY0-95), 206*sx, (GY1-GY0+190)*sy);
  tctx.strokeRect(X(H1)-206*sx, Y(GY0-95), 206*sx, (GY1-GY0+190)*sy);
  tctx.strokeRect(X(H0), Y(GY0-28), 74*sx, (GY1-GY0+56)*sy);
  tctx.strokeRect(X(H1)-74*sx, Y(GY0-28), 74*sx, (GY1-GY0+56)*sy);

  [[H0+150,CY],[H1-150,CY]].forEach(([px,py])=>{
    tctx.beginPath(); tctx.arc(X(px),Y(py),4,0,Math.PI*2); tctx.fillStyle='#fff'; tctx.fill();
  });
  tctx.beginPath(); tctx.arc(X(H0+150),Y(CY),79*sx,-0.65,0.65); tctx.stroke();
  tctx.beginPath(); tctx.arc(X(H1-150),Y(CY),79*sx,Math.PI-0.65,Math.PI+0.65); tctx.stroke();

  tctx.lineWidth=2.4;
  tctx.beginPath(); tctx.arc(X(H0),Y(V0),17*sx,0,Math.PI/2); tctx.stroke();
  tctx.beginPath(); tctx.arc(X(H1),Y(V0),17*sx,Math.PI/2,Math.PI); tctx.stroke();
  tctx.beginPath(); tctx.arc(X(H0),Y(V1),17*sx,-Math.PI/2,0); tctx.stroke();
  tctx.beginPath(); tctx.arc(X(H1),Y(V1),17*sx,Math.PI,Math.PI*1.5); tctx.stroke();
...
function drawMinimap(){
  ...
  octx.strokeStyle='rgba(255,255,255,.6)'; octx.lineWidth=1.4;
  octx.strokeRect(mx,my,mw,mh);
  octx.beginPath(); octx.moveTo(mx+mw/2,my); octx.lineTo(mx+mw/2,my+mh); octx.stroke();
  octx.beginPath(); octx.arc(mx+mw/2,my+mh/2, mh*0.16,0,Math.PI*2); octx.stroke();

  const sx=mw/(H1-H0), sy=mh/(V1-V0);
  function mproj(x,y){ return {x:mx+(x-H0)*sx, y:my+(y-V0)*sy}; }

  // 카메라가 실제로 보고 있는 대략적인 영역 표시
  const camGX = camPosX/SCALE3D + CX, camGY = camPosZ/SCALE3D + CY;
  const halfW=(H1-H0)*0.155, halfD=(V1-V0)*0.155;
  const vp1=mproj(camGX-halfW, camGY-halfD), vp2=mproj(camGX+halfW, camGY+halfD);
  octx.strokeStyle='rgba(255,255,255,.85)'; octx.lineWidth=1.2;
  octx.strokeRect(vp1.x, vp1.y, vp2.x-vp1.x, vp2.y-vp1.y);

  for(const p of red){
    const q=mproj(p.x,p.y);
    octx.beginPath(); octx.arc(q.x,q.y,1.8,0,Math.PI*2); octx.fillStyle='#ff4757'; octx.fill();
  }
  for(const p of blue){
    const q=mproj(p.x,p.y);
    const isAct=(p===activePlayer());
    if(isAct){
      octx.beginPath(); octx.arc(q.x,q.y,3.2,0,Math.PI*2);
      octx.strokeStyle='rgba(255,212,0,.9)'; octx.lineWidth=1; octx.stroke();
    }
    octx.beginPath(); octx.arc(q.x,q.y,1.8,0,Math.PI*2); octx.fillStyle=isAct?'#ffd400':'#2ea8ff'; octx.fill();
  }
  const bq=mproj(ball.x,ball.y);
  octx.beginPath(); octx.arc(bq.x,bq.y,3.4,0,Math.PI*2);
  octx.fillStyle='rgba(255,255,255,.35)'; octx.fill();
...
function drawPlayerNames(){
  const drawOne=(p)=>{
    ...
    const isActive = p.team==='B' && p===activePlayer();
    const fontSize = Math.max(9.5, 11.5*P.scale);
    octx.font=`800 ${fontSize}px Rajdhani, sans-serif`;
    octx.textAlign='center';
    const label = p.pname;
    const ty = P.y - 27*Math.max(P.scale,0.7);

    // 가독성용 반투명 배경 칩
    const tw = octx.measureText(label).width;
    octx.fillStyle = p.team==='B' ? 'rgba(14,95,168,.55)' : 'rgba(168,32,44,.55)';
    octx.fillRect(P.x-tw/2-4, ty-fontSize*0.78, tw+8, fontSize*1.05);

    octx.fillStyle = isActive? '#ffd400' : '#ffffff';
...
function loop(ts){
  ...
  if(gameState==='playing'){
    if(now()>=freezeUntil){
      ...
      timeLeft-=dt;
      if(timeLeft<=90 && !halftimeDone){
        halftimeDone=true;
        triggerHalftime();
      } else if(timeLeft<=0){ timeLeft=0; endMatch(); }
...
function triggerHalftime(){
  ...
  document.getElementById('ht-score').textContent = score.B+' : '+score.R;
  renderHalftime();
  document.getElementById('halftimeOverlay').style.display='flex';
  commentate('⏱️ 전반전 종료! 잠시 휴식합니다.');
}
function benchCandidatesFor(role){
  if(blueRosterSource!=='career' || !careerState) return [];
  const onFieldIds = new Set(blue.map(p=>p.squadId));
  return careerState.squad.filter(p=> p.role===role && !onFieldIds.has(p.id) && !benchUsedIds.has(p.id) && !isInjured(p));
}
function performSub(outSlotIdx, benchPlayerId){
  if(subsUsedThisMatch>=SUBS_MAX) return '교체 카드를 모두 사용했어 (최대 3장).';
  const outP = blue[outSlotIdx];
  if(!outP) return '교체 대상을 찾을 수 없어.';
  const benchP = careerState.squad.find(s=>s.id===benchPlayerId);
  if(!benchP) return '후보 선수를 찾을 수 없어.';
  if(benchP.role!==outP.role) return '같은 포지션끼리만 교체할 수 있어.';
  const newPlayer = {
    id:outP.id, team:'B', num:outP.num, role:outP.role, isGK:outP.isGK,
    baseFx:outP.baseFx, baseFy:outP.baseFy,
    x:outP.x, y:outP.y, vx:0, vy:0, facing:{x:1,y:0},
  ...
  if(wasActive) activeBlueIdx=outSlotIdx;
  benchUsedIds.add(benchPlayerId);
  if(outP.squadId) benchUsedIds.add(outP.squadId);
  subsUsedThisMatch++;
  commentate(`🔄 선수 교체: ${outP.pname||'선수'} → ${benchP.name}`);
  return null;
}
function renderHalftime(pickOutSlot){
  const isCareer = blueRosterSource==='career' && careerState;
  const subsLeft = SUBS_MAX-subsUsedThisMatch;

  if(isCareer && pickOutSlot!==null && pickOutSlot!==undefined){
    const role=blue[pickOutSlot].role;
    const cands=benchCandidatesFor(role);
    const rows = cands.length? cands.map(p=>`<tr><td class="tname">${p.name}</td><td>${p.age}</td><td><b>${ovrOf(p)}</b></td>
      <td><div class="mini-btn" data-action="ht_sub_confirm" data-slot="${pickOutSlot}" data-id="${p.id}">교체 투입</div></td></tr>`).join('')
      : `<tr><td colspan="4" style="color:#8aa;text-align:center;">같은 포지션의 후보 선수가 없어.</td></tr>`;
    document.getElementById('htBody').innerHTML = `
      <div class="meta-sub">${blue[pickOutSlot].pname||'선수'} 를 아웃시키고 투입할 후보를 골라줘.</div>
      <table class="meta-table"><tr><th>이름</th><th>나이</th><th>OVR</th><th></th></tr>${rows}</table>
      <div class="ghost-btn" data-action="ht_back">◀ 뒤로</div>
    `;
    return;
  }
  ...
  let subUI='';
  if(isCareer){
    const rows = blue.filter(p=>!p.isGK).map((p,idx)=>{
      const realIdx = blue.indexOf(p);
      return `<tr><td class="tname">${p.pname||('#'+p.num)}</td><td>${p.role}</td><td><b>${p.ovr||ovrOf({attrs:p.attrs})}</b></td>
        <td><div class="mini-btn ${subsLeft<=0?'disabled':''}" data-action="ht_pick_out" data-slot="${realIdx}">교체</div></td></tr>`;
    }).join('');
    subUI = `
      <div class="meta-sub" style="margin-top:8px;">선수 교체 (남은 카드: <b style="color:var(--gold);">${subsLeft}</b>/${SUBS_MAX})</div>
      <table class="meta-table"><tr><th>선수</th><th>포지션</th><th>OVR</th><th></th></tr>${rows}</table>
    `;
  }

  document.getElementById('htBody').innerHTML = `
    <div class="meta-sub">전반 슈팅 ${stats.shots}회 · 태클 ${stats.tackles}회. 후반전 전술을 정비해줘.</div>
    <div class="diff-grid">
      <div class="opt-btn ${tactic==='attacking'?'sel':''}" data-action="ht_tactic" data-tactic="attacking">⚔️ 공격적</div>
      <div class="opt-btn ${tactic==='balanced'?'sel':''}" data-action="ht_tactic" data-tactic="balanced">⚖️ 균형</div>
      <div class="opt-btn ${tactic==='defensive'?'sel':''}" data-action="ht_tactic" data-tactic="defensive">🛡️ 수비적</div>
    </div>
    ${subUI}
    <button class="continue-btn" data-action="ht_resume" style="margin-top:10px;">▶️ 후반전 시작</button>
  `;
}
...
function endMatch(){
  ...
  else if(metaMode==='continental') label = continentalOnMatchEnd(b,r,win) || label;
...
document.getElementById('retryBtn').addEventListener('click',()=>{
  ...
  else if(metaMode==='continental') renderCareerContinental();
  else renderHub();
});
