...
def render(market, nw):
    ...
    if state == 'betting':
        ...
        <div style='text-align:center;padding:30px;background:linear-gradient(135deg,rgba(180,0,0,0.15),rgba(0,100,0,0.15));
          ...
          <div style='color:#B0BAC8;margin-top:10px;font-size:0.88rem;'>블랙잭(A+10) = 베팅의 1.5배 추가 지급 &nbsp;|&nbsp; 더블다운 = 카드 1장 추가 + 베팅 2배 &nbsp;|&nbsp; 스플릿 = 페어 분리</div>
        </div>
    ...
    # ── 결과 화면 ──
        ...
        def show_result_card(hand, hand_bet, label):
            ...
            <div style='text-align:center;background:rgba(0,0,0,0.4);border:2px solid {res_col};
              ...
              <div style='color:#B0BAC8;font-size:0.8rem;margin-top:6px;'>지급액: {format_korean_money(prize)}</div>
            </div>
            """, unsafe_allow_html=True)
            return prize
