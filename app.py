...
# ==============================
st.set_page_config(page_title="DOPAHYOMIN UNIVERSE v18.2", page_icon="🌌", layout="wide", initial_sidebar_state="collapsed")
...
.login-title { font-family:'Orbitron',monospace !important; font-size:clamp(2rem,6vw,4rem) !important; font-weight:900; text-align:center; background:linear-gradient(135deg,#FFD700 0%,#FF8C00 50%,#FF4500 100%); -webkit-background-clip:text; -webkit-text-fill-color:transparent; padding:20px 0 5px 0; letter-spacing:2px; animation:glow 3s ease-in-out infinite alternate; }
@keyframes glow { from{filter:drop-shadow(0 0 10px #FFD700)} to{filter:drop-shadow(0 0 30px #FF4500)} }
.login-sub { text-align:center; color:#888 !important; font-size:1rem; margin-bottom:20px; letter-spacing:3px; }
</style>""", unsafe_allow_html=True)

    st.markdown("<div class='login-title'>🌌 DOPAHYOMIN UNIVERSE</div>", unsafe_allow_html=True)
    st.markdown(f"<div class='login-sub'>∙ 자본주의 생존 시뮬레이션 시즌 {market.get('season_num', 1)} ∙</div>", unsafe_allow_html=True)

    # ==============================
    # 🌠 도메인 공지 및 게임 소개 스플래시 패널
    # ==============================
    st.markdown(f"""
<div style='background: linear-gradient(135deg, rgba(15, 20, 35, 0.9), rgba(5, 10, 15, 0.95)); border: 1px solid rgba(255, 215, 0, 0.3); border-radius: 16px; padding: 30px; margin-bottom: 20px; max-width: 850px; margin-left: auto; margin-right: auto; box-shadow: 0 10px 30px rgba(255, 215, 0, 0.1);'>
    <div style='text-align: center; margin-bottom: 25px;'>
        <div style='font-size: 3.5rem; margin-bottom: 10px;'>🌌</div>
        <h2 style='color: #FFD700; margin: 0; font-family: "Orbitron", sans-serif; letter-spacing: 2px;'>WELCOME TO DOPAHYOMIN UNIVERSE</h2>
    ...
    <div style='display: flex; gap: 20px; flex-wrap: wrap;'>
        <div style='flex: 1; min-width: 300px; background: rgba(255, 75, 75, 0.05); border: 1px solid rgba(255, 75, 75, 0.4); border-radius: 12px; padding: 20px;'>
            <h4 style='color: #FF4B4B; margin-top: 0; display: flex; align-items: center; gap: 8px; font-size: 1.1rem;'>🚨 시스템 대공사 및 전용 도메인 안내</h4>
            <p style='color: #E2E8F0; font-size: 0.85rem; line-height: 1.7; margin-bottom: 10px;'>
            데이터베이스를 <b>외부 클라우드(MongoDB)로 완벽 분리</b>하고 <b>37개 모듈화 설계</b>를 적용하여 서버 안정성을 극대화했습니다. 유저 자산은 이제 영구히 안전합니다!
            </p>
            <div style='background: rgba(75, 158, 255, 0.1); border-left: 4px solid #4B9EFF; padding: 10px 15px; border-radius: 0 8px 8px 0;'>
                <div style='color: #4B9EFF; font-weight: bold; font-size: 0.85rem; margin-bottom: 3px;'>🔗 공식 포털 접속 주소</div>
                <div style='color: #FFF; font-size: 1.1rem; font-weight: 900; letter-spacing: 1px;'>dopahyomin.site</div>
            </div>
        </div>
        <div style='flex: 1; min-width: 300px; background: rgba(255, 214, 0, 0.05); border: 1px solid rgba(255, 214, 0, 0.4); border-radius: 12px; padding: 20px;'>
            <h4 style='color: #FFD600; margin-top: 0; display: flex; align-items: center; gap: 8px; font-size: 1.1rem;'>🏆 정규 시즌 1 공식 개막</h4>
            <p style='color: #E2E8F0; font-size: 0.85rem; line-height: 1.7; margin-bottom: 0;'>
            <b>[시즌 기간]</b> 2026.04.15 ~ 05.15<br>
...
""", unsafe_allow_html=True)
    ...
    # ==============================
    # 🔍 시스템 아키텍처 상세 안내 (라우팅 추가)
    # ==============================
    st.markdown("""
<div style='background: rgba(255, 215, 0, 0.03); border: 1px dashed rgba(255, 215, 0, 0.3); border-radius: 12px; padding: 20px; max-width: 850px; margin: 0 auto 35px auto;'>
    <div style='display: flex; align-items: center; gap: 10px; margin-bottom: 15px;'>
        <span style='font-size: 1.5rem;'>🔍</span>
        <h4 style='color: #FFD700; margin: 0; font-family: "Orbitron", sans-serif; font-size: 1rem;'>SYSTEM ARCHITECTURE & MODULES</h4>
    </div>
    <div style='display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 15px;'>
        <div style='background: rgba(255, 255, 255, 0.03); padding: 12px; border-radius: 8px;'>
            ...
            <div style='color: #94A3B8; font-size: 0.75rem; line-height: 1.5;'>MongoDB Atlas 클라우드를 통한 자산 실시간 백업 및 SHA-256 암호화 보안 적용.</div>
        </div>
        <div style='background: rgba(255, 255, 255, 0.03); padding: 12px; border-radius: 8px;'>
            <div style='color: #00FF88; font-weight: 800; font-size: 0.85rem; margin-bottom: 5px;'>⚙️ Modular Logic</div>
            <div style='color: #94A3B8; font-size: 0.75rem; line-height: 1.5;'>37개 독립 모듈 구조로 경제 시뮬레이션 및 미니게임 로직의 안정성 극대화.</div>
        </div>
        <div style='background: rgba(255, 255, 255, 0.03); padding: 12px; border-radius: 8px;'>
            <div style='color: #00E5FF; font-weight: 800; font-size: 0.85rem; margin-bottom: 5px;'>🔄 Market Sync</div>
            <div style='color: #94A3B8; font-size: 0.75rem; line-height: 1.5;'>사용자 부재 시에도 시간당 주가/이자/임대료를 자동 계산하는 타임 슬롯 시스템.</div>
        </div>
        <div style='background: rgba(255, 255, 255, 0.03); padding: 12px; border-radius: 8px;'>
            <div style='color: #B554FF; font-weight: 800; font-size: 0.85rem; margin-bottom: 5px;'>🌐 Global Routing</div>
            <div style='color: #94A3B8; font-size: 0.75rem; line-height: 1.5;'>전용 도메인 기반 유동 포워딩 네트워크를 통한 최적화된 라우팅 환경 구축.</div>
        </div>
    </div>
    <div style='margin-top: 15px; padding: 10px; border-top: 1px solid rgba(255,255,255,0.05); text-align: center;'>
        <p style='color: #64748B; font-size: 0.7rem; margin: 0;'>Core Engine: Python 3.10+ | Framework: Streamlit v18.2 Modular Edition</p>
    </div>
</div>
""", unsafe_allow_html=True)
...
if is_pc_mode:
    with st.sidebar:
        st.markdown(f"<div style='padding:14px; background:rgba(255,255,255,0.05); border:1px solid #FFD700; border-radius:10px; margin-bottom:14px;'><div style='color:#FFD700;font-size:0.8rem;'>{st.session_state.equipped_title}</div><div style='font-size:1rem;font-weight:700;'>{st.session_state.logged_in_user}{unread_txt}</div></div>", unsafe_allow_html=True)
    ...
    with c1: st.markdown(f"<div style='font-size:0.9rem;'>👤 <b>{st.session_state.logged_in_user}</b>{unread_txt} | <span style='color:#FFD700;'>{st.session_state.equipped_title}</span><br>💵 {format_korean_money(st.session_state.global_cash)}</div>", unsafe_allow_html=True)
    with c2: 
