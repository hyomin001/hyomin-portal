...
<div id="depgraph-root" style="background:#060a14;border:1px solid rgba(255,255,255,0.08);border-radius:14px;padding:18px;">
  ...
  <div style="margin-top:14px;display:flex;gap:14px;flex-wrap:wrap;font-size:0.78rem;color:#8899bb;">
    ...
    <span><span style="display:inline-block;width:10px;height:10px;border-radius:3px;background:#00d4ff;margin-right:5px;"></span>UI 컴포넌트 (components)</span>
...
(function() {
  ...
  const labels = {
    ...
    'utils.market_sync':'market_sync.py','utils.css':'css.py','components.promo_popup':'promo_popup.py',
  ...
  const isComponent = m => m.startsWith('components.');
  ...
  function colorFor(m) {
    ...
    if (isComponent(m)) return { fill: 'rgba(0,212,255,0.12)', stroke: '#00d4ff', text: '#6ee2ff' };
